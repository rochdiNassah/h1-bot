...
<nav class="border-gray-200 px-2 sm:px-4 py-2.5 rounded ">
    <div class="container flex flex-wrap justify-between items-center mx-auto">
        <a href="<?= url('/') ?>" class="flex items-center">
            <img src="<?= url('logo.png') ?>" class="mr-3 h-6 sm:h-9" alt="Flowbite Logo" />
            <span class="text-gray-500 self-center text-xl whitespace-nowrap"><?= config('APP_NAME') ?></span>
        </a>
        ...
        <ul class="flex flex-col mt-4 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium">
            ...
            <a href="<?= $link['href'] ?>" class="block py-2 pr-4 pl-3 <?= trim(url(app('request')->path()), '\\/') === trim($link['href'], '\\/') ? 'text-amber-400' : 'text-gray-500' ?> border-b border-gray-100 hover:bg-gray-50 md:hover:bg-transparent md:border-0 md:hover:text-amber-500 md:p-0">
                <?= $link['text'] ?>
            </a>
            <?php endforeach; ?>
        </ul>
